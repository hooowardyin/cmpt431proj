  ...
  for(int y = T->sLength - 1; y >= 0 ; y--){
    for (int x = startCol ; x <= endCol; x++){
      if(y > x){
        ...
        // other diagonals (other than the first and second non-zero diagonals)
        else{
          int leftBot;
          int left;
          int toBeSent;
...
void longestPalindromeSubseq(DPTable *T) {
  ...
  std::cout << "process: " << worldRank <<", the start col: "<< startCol << 
  ", the end col: " << endCol <<" ,time using(in seconds): " << subTimer.stop()<<"\n" ;
